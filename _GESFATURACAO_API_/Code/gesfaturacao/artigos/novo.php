...
?>
<html>
	<title>Novo Artigo | GESFaturação</title>
	<head>
	...
	<body>
		<div class="container">
			...
			<!-- Confirm Category Association to all POS Terminals -->
			<div id="modal-categoria" tabindex="-1" role="dialog" aria-labelledby="modal-header-primary-label" aria-hidden="true" class="modal fade">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header modal-header-orange">
							...
							<h4 id="modal-header-primary-label" class="modal-title">Categoria</h4>
						</div>
							<div class="modal-body" style="overflow-y: visible;">
								<div class="row">
									<div class="col-md-12">
										<p>A categoria selecionada não se encontra associada aos postos.</p>
										<p>Deseja continuar e associar a categoria a todos os postos?</p>
									</div>
								</div>
							</div>
							<div class="modal-footer">
								<button class="btn btn-yellow submit_categoria" type="button">
												...
												<div class="row mtm">
													...
													<div class="col-md-4">
														<div class="form-group">
															...
															<input type="text" class="form-control percentage" id="retencao_percenteagem_artigo" name="retencao_percenteagem_artigo" placeholder="0 %" style="display: none;" />
														</div>
													</div>
												</div>
												...
												<div class="row">
													<div class="col-xs-12">
														<table class="table table-bordered linesPrices" id="table_prices" style="margin-bottom: 5px;">
															...
															<tbody id="tbody_prices">
																<tr class="articles_prices_lines" id="price_line_0">
																	<td style="min-width:200px;">
																		<select data-id-line="0" class="lineInputImposto form-control select2 show-tick" data-live-search="true"  data-header="Selecione Imposto" id="iva_linha_0" name="iva_linha_0" >
																			<option value="" data-hidden="true">Imposto</option>
																		</select>
																	</td>
																	<td style="min-width:200px;">
																		<input type="text" data-id-line="0" class="lineInputPrecoPVP form-control moneyBig" id="precopvp_linha_0" name="precopvp_linha_0" placeholder="0,00 €" />
																	</td>
																	<td style="min-width:200px;">
																		<input type="text" data-id-line="0" class="lineInputPreco form-control moneyBig" id="preco_linha_0" name="preco_linha_0" placeholder="0,00 €" />
																	</td>
																	<td style="width:30px; text-align: center;">
																		<a class="options-link text-red" onclick="deletePriceLine(0)" title="Remover Linha"><i class="trashLineIcone fa fa-trash"></i></a>
																	</td>
																</tr>
															</tbody>
														</table>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$("#retencao_percenteagem_artigo").prop('disabled', true);
				...
				var defaultTipo = "P";
				var defaultStock = "0";
				$('#tipo_artigo').val(defaultTipo);
				$("#tipo_artigo").selectpicker('refresh');
				$('#stock_artigo').val(defaultStock);
			...
			$("#imposto_artigo").on('change', function(event){
				...
				if(taxa == 0) {
					...
					if($('#motivo_isencao_artigo').attr("data-predefinido")) {
						$('#motivo_isencao_artigo').selectpicker('val', $('#motivo_isencao_artigo').attr("data-predefinido"));
					}
				} else {
			...
			$("#tipo_artigo").on('change', function(event){
				...
				if(tipoArtigo == 'S'){
					...
					$("#retencao_percenteagem_artigo").prop('disabled', false);
				}else{
					$("#retencao_select_artigo").prop('disabled', true);
					$("#retencao_select_artigo").selectpicker('refresh');
					$("#retencao_valor_artigo").autoNumeric('set',0);
					$("#retencao_valor_artigo").prop('disabled', true);
					$("#retencao_percenteagem_artigo").autoNumeric('set',0);
					$("#retencao_percenteagem_artigo").prop('disabled', true);
				}
			});
			...
			$("#retencao_select_artigo").on('change', function(event){
				...
				if(tipo == 'Valor'){
					...
					$('#retencao_percenteagem_artigo').hide();
					$('#retencao_percenteagem_artigo').autoNumeric('set', 0);
				}else{
					$('#retencao_percenteagem_artigo').show();
			...
			$.validator.addMethod("postosRequired", function (value, element) {
				var addPostos = $("#adicionar_posto").val()
				if(addPostos == "1") {
					return false;
				}
				return true;
			...
			function submitData(fotoPath) {
				...
				if (valid == false) {
					...
					$.ajax({
						type : 'POST',
						url : '/gesfaturacao/server/artigos/artigos_inserir.php',
						dataType : 'json',
						data : {
						...
						complete : function(resposta) {
							...
							if (resp.responseJSON.errors) {
								...
								}else if(resp.responseJSON.categorias_em_falta){
								...
								setTimeout(function() {
									window.location.href = "../artigos/detalhes?artigo="+resp.responseJSON.idArtigo;
								}, 1000);
							}
			...
			$(document).ready(function() {
				$(window).keydown(function(event){
					if(event.keyCode == 13 && document.activeElement.classList.contains('codigobarras_artigo')) {
