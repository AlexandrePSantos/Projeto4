					...
					(function($) {
						...
						'margin-right' : 'auto',
								...
								canvas = create_canvas_for(this, options.width, options.height);
								$(canvas).css(canvas_style);
								canvas.height = options.height;
								canvas.width = options.width;

								$(map).trigger('mouseout.maphilight, blur.maphilight');
									...
									$(map).find('area[coords]').each(function() {
										...
										if (area_options.alwaysOn) {
											if (!canvas_always && has_canvas) {
												canvas_always = create_canvas_for(img[0], options.width, options.height);
												$(canvas_always).css(canvas_style);
												canvas_always.width = options.width;
												canvas_always.height = options.height;
