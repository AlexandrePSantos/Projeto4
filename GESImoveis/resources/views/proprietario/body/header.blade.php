...
<div class="col-md-12 mts">
    <div class="row">
        ...
        <div class="col-md-3 mtl">
            ...
            @endphp
            <p>Bem Vindo ao GESImoveis, <a href="{{ route('proprietario.profile') }}" aria-haspopup="true" aria-expanded="false">{{ $profileData->name }}</a><p>
            {{-- <p>
                <a class="options-link text-red" href="{{ route('proprietario.logout') }}">Sair</a>
            </p> --}}
            <a class="options-link text-red" href="{{ route('proprietario.logout') }}">Sair</a>
            <form method="POST" action="{{ route('proprietario.logout') }}">
                @csrf
                <button type="submit" class="options-link text-red">Sair</button>
            </form>
            <!-- NAVBAR -->
            <div class="col-md-12">
